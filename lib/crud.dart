...
class crudMethods{
  ...
  getShopName(cid) async{
    return await Firestore.instance.collection('shops').document(cid).get();
  }
  ...
  Future<void>addHistory(id,data) async{
    for(var i=0;i<data.length;i=i+1) {
      Firestore.instance.collection('History').document(id).collection('items1').add(data[i])
          .catchError((e) {
        print(e);
      });
    }
  ...
  getHistory(id) async{
    return await Firestore.instance.collection('History').document(id).collection('items1').getDocuments();
  }
  ...
  Future<void>updateShop(shopid,a) async{
    ...
    for(var i=0;i<a.length;i=i+1) {
      Firestore.instance.collection('shops').document(shopid).collection('item1')
